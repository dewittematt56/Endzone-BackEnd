...
def __run_ingame_report__(team_of_interest: str, game_ID: str, user_team_code: str):
    with thread_lock:
        report = IngameReport(team_of_interest, game_ID, user_team_code)
        pdf = report.combine_reports()
        return pdf
...
def ingame_report_run():
    try:
        ...
        executor_job = report_executor.submit(__run_ingame_report__, requestedTeamOfInterest, requestedGame, current_team)
        response = executor_job.result()
        return send_file( 
                io.BytesIO(response),
                mimetype='application/pdf',
